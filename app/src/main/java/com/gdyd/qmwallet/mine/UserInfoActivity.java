...
//    private String BigBankCode;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ID = loginInfoBean.getUserData().getMerchant().getID();
        phone = loginInfoBean.getUserData().getMerchant().getPhoneNumber();
        ...
        checkState = loginInfoBean.getUserData().getMerchant().getCheckState();
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (name==null||name.equals("")){
                    Toast.makeText(UserInfoActivity.this, "收款人姓名不能为空", Toast.LENGTH_SHORT).show();
                    return;
                }
                String id = write_sk_id.getText().toString();
                if (id==null||id.equals("")||id.length()!=18){
                    Toast.makeText(UserInfoActivity.this, "收款人身份证不正确", Toast.LENGTH_SHORT).show();
                    return;
                }
                String phone = write_sk_phone.getText().toString();
                if (phone==null||phone.equals("")||phone.length()!=11){
                    Toast.makeText(UserInfoActivity.this, "预留手机不正确", Toast.LENGTH_SHORT).show();
                    return;
                }
                String site = write_sk_site.getText().toString();
                if (site==null||site.equals("")){
                    Toast.makeText(UserInfoActivity.this, "详细地址不能为空", Toast.LENGTH_SHORT).show();
                    return;
                }
                String number = write_bank_number.getText().toString();
                if (number==null||number.equals("")){
                    Toast.makeText(UserInfoActivity.this, "银行卡号不能为空", Toast.LENGTH_SHORT).show();
                    return;
                }else if (number.length()<16){
                    Toast.makeText(UserInfoActivity.this, "银行卡号最少为16位", Toast.LENGTH_SHORT).show();
                    return;
                }
                String type = write_bank_type.getText().toString();
                if (type==null||type.equals("")){
                    Toast.makeText(UserInfoActivity.this, "请选择所属银行", Toast.LENGTH_SHORT).show();
                    return;
                }
                String ssq = write_ssq.getText().toString();
                if (ssq==null||ssq.equals("")){
                    Toast.makeText(UserInfoActivity.this, "请选择开户地址", Toast.LENGTH_SHORT).show();
                    return;
                }
                String psw_two = write_psw_two.getText().toString();
                if (psw_two==null||psw_two.equals("")){
                    Toast.makeText(UserInfoActivity.this, "请选择所属支行", Toast.LENGTH_SHORT).show();
                    return;
                }
                Log.d("zanZQ", "onClick: "+cityId+"pro"+provinceId);
                if (checkState ==3||checkState==0||checkState==5){
                    if (cityId==0||provinceId==0){
                        Toast.makeText(UserInfoActivity.this, "请重新选择开户地址", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    tip_ddsq.setVisibility(View.VISIBLE);
                    backgroundAlpha(0.7f);
                 UserInfoBean.MerchantBean merchant=new UserInfoBean.MerchantBean(ID,cityId+"",site,"",name,name,"",provinceId+"","0",id,phone);
                 UserInfoBean.BankInfoBean banInfo=new UserInfoBean.BankInfoBean(psw_two,bankcode,OPEN_STLNO,number,cityId+"",provinceId+"",1,name);
                    long transTyp = EncryptionHelper.getDate();
                    String transTy="1046"+transTyp+"";
                    String transKe = EncryptionHelper.md5(transTy);
                    UserInfoBean userInfoBean = new UserInfoBean(1046, transKe, merchant, banInfo,transTyp);
                    Log.d("zanZQ", "onClick: merchant "+merchant.toString());
                    Log.d("zanZQ", "onClick: banInfo "+banInfo.toString());
                    Log.d("zanZQ", "onClick: userInfoBean "+userInfoBean);
                    presenter.getSubmitInfo(userInfoBean);
             }else{
                 startActivity(new Intent(UserInfoActivity.this, MainActivity.class).putExtra(APPConfig.LOGIN_INFO, loginInfoBean));
                 finish();
             }
           }
        });
        ...
        layout_bank_type.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                KeyBoardUtils.closeKeybordText(write_bank_type,UserInfoActivity.this);
            ...
            public void onClick(View v) {
                ...
                if (bankcode !=null&&cityName!=null){
                  //  presenter.getBranchBank(bankcode,cityName);
                    Intent intent = new Intent(UserInfoActivity.this,ZhActivity.class);
        ...
        layout_bank_site.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
               ChangeAddressPopwindow pw = new ChangeAddressPopwindow(UserInfoActivity.this);
                pw.setAddress(province,cityName,"");
                pw.setOutsideTouchable(true);
                pw.setAddresskListener(UserInfoActivity.this);
...
//        pw.setFocusable(true);
    ...
    public void ISumBankInfoView(BlankBean blankBean) {
     ...
     ArrayList<String> a=new ArrayList<>();
        ...
        createPw();
        wvCitys.setViewAdapter(new AddressTextAdapter(UserInfoActivity.this,a,0));
        backgroundAlpha(0.7f);
        //pw.showAsDropDown(write_ssq, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, -16, getResources().getDisplayMetrics()), 20);
        ...
        try {
         ...
         //   Toast.makeText(this, sMessage, Toast.LENGTH_SHORT).show();
            if (nResul==1){
                ...
                if (status.equals("0")){
                     Toast.makeText(this, sMessage, Toast.LENGTH_SHORT).show();
                    startActivity(new Intent(UserInfoActivity.this, RealNameActivity.class).putExtra(APPConfig.LOGIN_INFO, loginInfoBean).putExtra("list",list));
                    finish();
                }else{
